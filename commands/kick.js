...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            return await interaction.reply({content: 'You don\'t have permissions to kick members.', ephemeral: true});

            if (!target_guild.kickable)
            return await interaction.reply({content: 'This target cannot be kicked.', ephemeral: true});

            guild.members.kick(target_guild, reason);
            return await interaction.reply(`${target.username} was kicked due to ${reason}`);
        }
        catch (error){
            console.log(error);
